...
function Bisection() {
  ...
  // ฟังก์ชันสำหรับดึงโจทย์จาก backend
  ...
  const calculateBisection = () => { // เปลี่ยนชื่อฟังก์ชันเป็น calculateBisection
    ...
    const results = []; // ใช้ตัวแปรนี้เก็บผลลัพธ์
    let xm, iterations = 1, error = Infinity; // แก้ไขการคำนวณค่าความผิดพลาด
    let currentXL = xlNum;
    let currentXR = xrNum;
    while (error >= errorValNum) {
      ...
      error = (Math.abs(currentXR - currentXL) / currentXR) * 100;
    ...
    setResults(results); // เก็บผลลัพธ์ลงใน state results
    setans(xm);
    setChatr(results);
  }
  const data = [
  ...
  const layout = {
    width: 500, height: 500, title: 'Bisection Method',
    xaxis: {
      title: {
        text: 'Iteration',
      },
    ...
    <>
      <div className="flex flex-col justify-center items-center">
        ...
        <div className="flex gap-6">
          <button className="btn btn-primary mt-5" onClick={calculateBisection}>
            คำนวณ
          </button>
